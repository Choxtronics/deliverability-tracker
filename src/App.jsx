...
const WORKER_COLORS = {
  Federico: { bg:"#EDE9FE", text:"#5B21B6", dot:"#7C3AED", btn:"#7C3AED", ring:"#7C3AED22" },
  Ahmed:    { bg:"#DBEAFE", text:"#1D4ED8", dot:"#2563EB", btn:"#2563EB", ring:"#2563EB22" },
  Khaled:   { bg:"#D1FAE5", text:"#065F46", dot:"#059669", btn:"#059669", ring:"#05966922" },
  Mai:      { bg:"#FCE7F3", text:"#9D174D", dot:"#DB2777", btn:"#DB2777", ring:"#DB277722" },
};
const FALLBACK = { bg:"#F3F4F6", text:"#374151", dot:"#9CA3AF", btn:"#6B7280", ring:"#6B728022" };
...
export default function App() {
  ...
  const saveEntry = taskId => {
    const f=form[taskId];
    setEntries(prev=>[...prev,{id:genId(),date:todayStr(),worker:activeWorker,taskId,status:f.status,changes:f.changes,notes:f.notes,ts:new Date().toISOString()}]);
  ...
  const submitUpdate = () => {
    ...
    if(editingUpdate){ setUpdates(prev=>prev.map(u=>u.id===editingUpdate?{...u,...updateForm,editedTs:new Date().toISOString()}:u)); setEditingUpdate(null); }
    else { setUpdates(prev=>[...prev,{...updateForm,id:genId(),ts:new Date().toISOString(),replies:[]}]); }
    setUpdateForm(blankUpdate()); setShowUpdateForm(false);
  };
  const addReply = (uid,text,worker) => { if(!text.trim()) return; setUpdates(prev=>prev.map(u=>u.id===uid?{...u,replies:[...(u.replies||[]),{id:genId(),worker,text,ts:new Date().toISOString()}]}:u)); };
  const changeUpdateStatus = (uid,s) => setUpdates(prev=>prev.map(u=>u.id===uid?{...u,status:s}:u));
  ...
  const generateDemoData = () => {
    ...
    for(let d=13;d>=0;d--){
      ...
      WORKERS.forEach(worker=>{[...TASKS].sort(()=>Math.random()-.5).slice(0,7+Math.floor(Math.random()*TASKS.length)).forEach(task=>{
        const ts=new Date(date); ts.setHours(8+Math.floor(Math.random()*9),Math.floor(Math.random()*60),0,0);
        demo.push({id:genId(),date:ds,worker,taskId:task.id,status:"OK",changes:chg[Math.floor(Math.random()*chg.length)],notes:nts[Math.floor(Math.random()*nts.length)],ts:ts.toISOString()});
      });});
    }
    setEntries(prev=>[...prev,...demo]);
  };

  const generateDemoUpdates = () => {
    const samples=[
      {type:"Experiment",worker:"Federico",title:"Testing IP warm-up on Cluster 4",description:"Started a gradual warm-up plan on Cluster 4 increasing daily send volume by 10% each day.",result:"After 5 days, bounce rate dropped from 6.2% to 3.8%.",status:"Ongoing",ipChangeType:"Added IPs",ipDetail:"Cluster 4",ipRange:"185.228.38.1-20",ipReason:"Warm-up plan",ipResult:"Bounce improving",extraIPs:[]},
      {type:"Change",worker:"Ahmed",title:"Switched tracking domain for EmailChef",description:"Replaced old tracking domain with a clean domain. Old domain had SURBL blacklist hits.",result:"No blacklist issues after 3 days.",status:"Completed",ipChangeType:"— None —",ipDetail:"",ipRange:"",ipReason:"",ipResult:"",extraIPs:[]},
      {type:"Fix",worker:"Mai",title:"Resolved queue buildup on Outside Cluster 2",description:"Cluster 2 had stuck queue due to misconfigured retry interval. Flushed 1,200 queued messages.",result:"Queue cleared within 20 minutes.",status:"Completed",ipChangeType:"Removed IPs",ipDetail:"Cluster 2",ipRange:"185.228.36.70-75",ipReason:"Stuck queue / retry misconfiguration",ipResult:"Queue cleared, normal delivery resumed",extraIPs:[]},
      {type:"Observation",worker:"Khaled",title:"Spike in MS 550 5.7.1 errors Monday mornings",description:"Microsoft bounce errors spike every Monday 08:00–10:00 UTC. Possibly related to weekend queue flushing.",result:"Still investigating.",status:"Ongoing",ipChangeType:"Changed Snooze Time",ipDetail:"Cluster 21",ipRange:"199.187.172.33-69",ipReason:"High MS complaint rate on Mondays",ipResult:"Snooze extended to 6h on weekends",extraIPs:[]},
      {type:"Experiment",worker:"Federico",title:"Cluster assignment by domain reputation",description:"Testing new assignment logic routing basic accounts to clusters based on domain reputation score.",result:"12% reduction in cross-contamination bounce events.",status:"Pending Results",ipChangeType:"Added IPs",ipDetail:"Cluster 140",ipRange:"185.228.39.100-150",ipReason:"Need isolated pool for reputation-based routing",ipResult:"Cluster created and accounts being migrated",extraIPs:[]},
    ];
    const now=Date.now();
    const demo=samples.map((s,i)=>({...s,id:genId(),ts:new Date(now-(samples.length-i)*14*60*60*1000).toISOString(),replies:[
      {id:genId(),worker:WORKERS[(i+1)%WORKERS.length],text:"Thanks for sharing — keeping an eye on this.",ts:new Date(now-(samples.length-i)*13*60*60*1000).toISOString()},
      ...(i%2===0?[{id:genId(),worker:WORKERS[(i+2)%WORKERS.length],text:"Good catch. Should we add this to the monthly report?",ts:new Date(now-(samples.length-i)*12*60*60*1000).toISOString()}]:[]),
    ]}));
    setUpdates(prev=>[...prev,...demo]);
  };

  const clearAll = () => { if(window.confirm("Clear ALL data?")){ setEntries([]); setUpdates([]); } };
    ...
    <div style={{minHeight:"100vh",background:"#F8FAFC",fontFamily:"system-ui,sans-serif"}}>
      ...
      <div style={{maxWidth:1100,margin:"0 auto",padding:"24px 16px"}}>
                    ...
                    <Card key={task.id} style={{marginBottom:8,border:isSaved?"1.5px solid #86EFAC":"1px solid #E5E7EB",padding:0,overflow:"hidden"}}>
                      <div style={{display:"flex",alignItems:"center",gap:12,padding:"12px 16px",flexWrap:"wrap"}}>
                        <div style={{flex:1,minWidth:140}}>
                          <div style={{fontSize:13,fontWeight:500,color:"#1E293B"}}>{task.name}</div>
                        </div>
                        ...
                        <div style={{borderTop:"1px solid #F1F5F9"}}>
                          ...
                          <div style={{padding:"14px 16px",background:wc(activeWorker).bg+"33"}}>
                            ...
                            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10}}>
                              ...
                              <div>
                                <label style={labelStyle}>Changes</label>
                                <input type="text" placeholder="e.g. Rotated IPs" value={form[task.id].changes} onChange={e=>updateField(task.id,"changes",e.target.value)} style={inputStyle}/>
                              </div>
                              <div>
                                <label style={labelStyle}>Notes</label>
                                <input type="text" placeholder="Any remarks…" value={form[task.id].notes} onChange={e=>updateField(task.id,"notes",e.target.value)} style={inputStyle}/>
                              </div>
                            </div>
            ...
            <Card style={{marginBottom:16,padding:"12px 16px"}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                ...
                {[["all","All time"],["today","Today"],["week","7 days"],["month","This month"],["last30","30 days"],["custom","Custom"]].map(([k,l])=>(
                  <button key={k} onClick={()=>applyUPreset(k)} style={{padding:"4px 10px",borderRadius:20,fontSize:12,fontWeight:500,cursor:"pointer",border:`1px solid ${filterUPreset===k?"#6366F1":"#E5E7EB"}`,background:filterUPreset===k?"#6366F1":"transparent",color:filterUPreset===k?"#fff":"#64748B",transition:"all 0.15s"}}>{l}</button>
                ))}
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {u.ipChangeType&&u.ipChangeType!=="— None —"&&(
                        <div style={{background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:8,padding:"10px 12px",marginTop:8}}>
                          ...
                          {(u.extraIPs||[]).map((ip,i)=>(ip.ipDetail||ip.ipRange)&&<div key={i} style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:4}}>
                            {ip.ipDetail&&<Pill label={`⊙ ${ip.ipDetail}`} bg="#EFF6FF" text="#1D4ED8"/>}
                            {ip.ipRange&&<span style={{fontSize:11,fontFamily:"monospace",padding:"2px 8px",borderRadius:20,background:"#F0FDF4",color:"#15803D",border:"1px solid #BBF7D0"}}>{ip.ipRange}</span>}
                          </div>)}
        ...
        {tab===2 && (
          <div>
            <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
              <button onClick={generateDemoData} style={{padding:"7px 14px",borderRadius:8,background:"#6366F1",border:"none",color:"#fff",fontSize:12,cursor:"pointer",fontWeight:500}}>Generate 2-week demo</button>
              <button onClick={generateDemoUpdates} style={{padding:"7px 14px",borderRadius:8,background:"#F59E0B",border:"none",color:"#fff",fontSize:12,cursor:"pointer",fontWeight:500}}>Generate demo updates</button>
              <button onClick={clearAll} style={{padding:"7px 14px",borderRadius:8,background:"#FEF2F2",border:"1px solid #FCA5A5",color:"#B91C1C",fontSize:12,cursor:"pointer",fontWeight:500}}>Clear all data</button>
            ...
            <Card style={{marginBottom:16,padding:"14px 18px"}}>
              <div style={{display:"flex",flexWrap:"wrap",gap:12,alignItems:"flex-end"}}>
                ...
                <div style={{marginLeft:"auto",textAlign:"right"}}>
                  <div style={{fontSize:12,fontWeight:500,color:"#1E293B"}}>{logRangeMode?`${fmtShort(logFrom)} → ${fmtShort(logTo)}`:fmtFull(logDate)}</div>
                  {logRangeMode&&<div style={{fontSize:11,color:"#94A3B8"}}>{logDays.length} days · {logFilteredEntries.length} entries</div>}
                </div>
              </div>
            </Card>
            ...
            {logFilteredEntries.length===0&&entries.length===0&&<div style={{textAlign:"center",padding:"64px 0",color:"#CBD5E1"}}><div style={{fontSize:32,marginBottom:8}}>📭</div><div>No logs yet</div></div>}
            {logFilteredEntries.length===0&&entries.length>0&&<div style={{textAlign:"center",padding:"48px 0",color:"#CBD5E1"}}><div style={{fontSize:28,marginBottom:8}}>📂</div><div>No entries for this period</div></div>}
            ...
            {currentPageDay&&(
              <div>
                {logRangeMode&&logDays.length>1&&<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}><span style={{fontSize:14,fontWeight:500,color:"#1E293B"}}>{fmtFull(currentPageDay)}</span><span style={{fontSize:12,color:"#94A3B8"}}>{currentDayEntries.length} entries</span><div style={{flex:1,height:1,background:"#F1F5F9"}}/></div>}
                {CATEGORIES.map(cat=>{
        ...
        {tab===3 && (()=>{
          ...
          const VIEWS=[["summary","Summary"],["cluster","Cluster updates"],["log","Full log"]];
            ...
            <div>
              <Card style={{marginBottom:16,padding:"16px 20px"}}>
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
                  {PRESETS.map(([k,l])=><button key={k} onClick={()=>applyRPreset(k)} style={{padding:"5px 12px",borderRadius:20,fontSize:12,fontWeight:500,cursor:"pointer",border:`1px solid ${rPreset===k?"#6366F1":"#E5E7EB"}`,background:rPreset===k?"#6366F1":"transparent",color:rPreset===k?"#fff":"#64748B",transition:"all 0.15s"}}>{l}</button>)}
                </div>
              ...
              <div style={{display:"flex",gap:6,marginBottom:20}}>
                {VIEWS.map(([k,l])=><button key={k} onClick={()=>setRView(k)} style={{padding:"7px 16px",borderRadius:8,fontSize:13,fontWeight:rView===k?500:400,cursor:"pointer",border:`1px solid ${rView===k?"#1E293B":"#E5E7EB"}`,background:rView===k?"#1E293B":"transparent",color:rView===k?"#fff":"#64748B",transition:"all 0.15s"}}>{l}</button>)}
              </div>
              {rView==="summary"&&(
                <div>
                    ...
                    {workerStats.map(({w,total,ok,days})=>{ const c=wc(w); const pct=total>0?Math.round((ok/total)*100):0; return (
                      <Card key={w} style={{padding:"14px 16px",borderLeft:`3px solid ${c.dot}`,borderRadius:"8px 12px 12px 8px"}}>
                        ...
                        <div style={{height:4,borderRadius:4,background:"#F1F5F9",overflow:"hidden"}}>
                          <div style={{width:`${pct}%`,height:4,background:c.dot,borderRadius:4,transition:"width 0.4s"}}/>
                        </div>
                  ...
                  <Card style={{padding:0,overflow:"hidden"}}>
                    <div style={{padding:"12px 16px",borderBottom:"1px solid #F1F5F9",fontSize:13,fontWeight:500,color:"#1E293B",display:"flex",justifyContent:"space-between"}}>
                      <span>Task health</span><span style={{fontSize:12,color:"#94A3B8",fontWeight:400}}>{rangeEntries.length} entries</span>
                    </div>
                      ...
                      <table style={{width:"100%",fontSize:12,borderCollapse:"collapse"}}>
                        ...
                        <tbody>{taskStats.map(t=>{ const pct=t.total>0?Math.round((t.ok/t.total)*100):0; const health=t.total===0?"—":t.ok===t.total?"●":"◑"; const hcolor=t.total===0?"#CBD5E1":t.ok===t.total?"#16A34A":"#D97706"; return (
                          <tr key={t.id} style={{borderTop:"1px solid #F8FAFC"}}>
                            <td style={{padding:"9px 14px",color:"#374151",fontWeight:500,maxWidth:200}}>{t.name}</td>
              ...
              {rView==="cluster"&&(()=>{
                const cUpdates=updates.filter(u=>u.ts.slice(0,10)>=rFrom&&u.ts.slice(0,10)<=rTo);
                if(cUpdates.length===0) return <div style={{textAlign:"center",padding:"64px 0",color:"#CBD5E1"}}><div style={{fontSize:32,marginBottom:8}}>⚗</div><div>No cluster updates in this period</div></div>;
                const byWorker=WORKERS.map(w=>({w,count:cUpdates.filter(u=>u.worker===w).length})).filter(x=>x.count>0);
                const byType=UPDATE_TYPES.map(t=>({t,count:cUpdates.filter(u=>u.type===t).length})).filter(x=>x.count>0);
                const byStatus=UPDATE_STATUS.map(s=>({s,count:cUpdates.filter(u=>u.status===s).length})).filter(x=>x.count>0);
                return <div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:10,marginBottom:16}}>
                    {[{l:"Total posts",v:cUpdates.length,c:"#6366F1"},{l:"Ongoing",v:cUpdates.filter(u=>u.status==="Ongoing").length,c:"#2563EB"},{l:"Completed",v:cUpdates.filter(u=>u.status==="Completed").length,c:"#16A34A"},{l:"Pending",v:cUpdates.filter(u=>u.status==="Pending Results").length,c:"#D97706"}].map(k=>(
                      <Card key={k.l} style={{padding:"14px 16px",background:"#F8FAFC",border:"none"}}>
                        <div style={{fontSize:26,fontWeight:500,color:k.c}}>{k.v}</div>
                        <div style={{fontSize:12,color:"#94A3B8",marginTop:2}}>{k.l}</div>
                      </Card>
                    ))}
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10,marginBottom:16}}>
                    {[{title:"By worker",rows:byWorker.map(({w,count})=>({dot:wc(w).dot,label:w,value:count,bg:wc(w).bg,text:wc(w).text}))},{title:"By type",rows:byType.map(({t,count})=>({dot:UPDATE_TYPE_META[t].text,label:t,value:count,bg:UPDATE_TYPE_META[t].bg,text:UPDATE_TYPE_META[t].text}))},{title:"By status",rows:byStatus.map(({s,count})=>({dot:UPDATE_STATUS_META[s]?.dot,label:s,value:count,bg:UPDATE_STATUS_META[s]?.bg,text:UPDATE_STATUS_META[s]?.text}))}].map(section=>(
                      <Card key={section.title} style={{padding:"14px 16px"}}>
                        <div style={{fontSize:11,fontWeight:500,color:"#94A3B8",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:10}}>{section.title}</div>
                        <div style={{display:"flex",flexDirection:"column",gap:8}}>
                          {section.rows.map(r=><div key={r.label} style={{display:"flex",alignItems:"center",gap:8}}>
                            ...
                            <span style={{fontSize:12,fontWeight:500,padding:"1px 8px",borderRadius:20,background:r.bg,color:r.text}}>{r.value}</span>
                          </div>)}
                        </div>
                      </Card>
                    ))}
                  ...
                  <Card style={{padding:0,overflow:"hidden"}}>
                    <div style={{padding:"12px 16px",borderBottom:"1px solid #F1F5F9",fontSize:13,fontWeight:500,color:"#1E293B",display:"flex",justifyContent:"space-between"}}>
                      <span>Posts in period</span><span style={{fontSize:12,color:"#94A3B8",fontWeight:400}}>{cUpdates.length}</span>
                    </div>
                    {[...cUpdates].reverse().map(u=>{ const c=wc(u.worker); const tm=UPDATE_TYPE_META[u.type]; const sm=UPDATE_STATUS_META[u.status]||UPDATE_STATUS_META["Ongoing"]; return (
                      <div key={u.id} style={{padding:"12px 16px",borderTop:"1px solid #F8FAFC",display:"flex",gap:10,alignItems:"flex-start"}}>
                        <Avatar name={u.worker} size={30}/>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:4}}>
                            <span style={{fontSize:12,fontWeight:500,color:c.text}}>{u.worker}</span>
                            <Pill label={`${tm.icon} ${u.type}`} bg={tm.bg} text={tm.text}/>
                            <Pill label={u.status} bg={sm.bg} text={sm.text}/>
                            {u.ipChangeType&&u.ipChangeType!=="— None —"&&<Pill label={u.ipChangeType} bg="#F1F5F9" text="#475569"/>}
                            <span style={{fontSize:11,color:"#CBD5E1",marginLeft:"auto"}}>{fmtShort(u.ts.slice(0,10))}</span>
                          </div>
                          <div style={{fontSize:13,fontWeight:500,color:"#1E293B"}}>{u.title}</div>
                          {u.description&&<div style={{fontSize:12,color:"#64748B",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{u.description}</div>}
                          {u.result&&<div style={{fontSize:12,color:"#92400E",marginTop:2}}>{u.result}</div>}
                        </div>
                      </div>
                    );})}
                  </Card>
                      ...
                      <tbody>
                        ...
                        {[...rangeEntries].reverse().map(e=>{ const task=TASKS.find(t=>t.id===e.taskId); const c=wc(e.worker); const sm=STATUS_STYLE[e.status]; return (
                          <tr key={e.id} style={{borderTop:"1px solid #F8FAFC"}}>
                            ...
                            <td style={{padding:"8px 14px",color:"#374151",maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={task?.name}>{task?.name}</td>
                            <td style={{padding:"8px 14px"}}><Pill label={e.status} bg={sm.bg} text={sm.text}/></td>
                            <td style={{padding:"8px 14px",color:"#64748B",maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={e.changes}>{e.changes||"—"}</td>
                            <td style={{padding:"8px 14px",color:"#64748B",maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={e.notes}>{e.notes||"—"}</td>
                          </tr>
                        );})}
                      </tbody>
                    </table>
        ...
        {tab===4 && (()=>{
          ...
          const getLog = (taskId, worker) => monthlyLogs[`${currentMonth}|${taskId}|${worker}`] || null;
          const saveMonthlyEntry = (taskId, worker, fields) => {
            setMonthlyLogs(prev=>({...prev,[`${currentMonth}|${taskId}|${worker}`]:{...fields,ts:new Date().toISOString()}}));
          };
          return (
            <div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:8}}>
                <div>
                  ...
                  <div style={{fontSize:12,color:"#94A3B8",marginTop:2}}>Recurring tasks to be completed once per month · <span style={{fontWeight:500,color:"#6366F1"}}>{monthLabel}</span></div>
                </div>
              </div>
                      ...
                      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
                        ...
                        <div style={{textAlign:"right",flexShrink:0}}>
                          ...
                          <div style={{width:80,height:4,borderRadius:4,background:"#EEF2FF",overflow:"hidden",marginTop:6,marginLeft:"auto"}}>
                            <div style={{width:`${pct}%`,height:4,background:pct===100?"#22C55E":"#6366F1",borderRadius:4,transition:"width 0.4s"}}/>
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      return (
                        <div key={w} style={{borderBottom:"1px solid #F8FAFC"}}>
                          <div style={{display:"flex",alignItems:"center",gap:12,padding:"11px 20px",flexWrap:"wrap"}}>
                            ...
                            {log
                              ? <Pill label={log.status} bg={log.status==="Done"?"#DCFCE7":log.status==="In Progress"?"#EFF6FF":"#F9FAFB"} text={log.status==="Done"?"#166534":log.status==="In Progress"?"#1D4ED8":"#94A3B8"}/>
                              : <Pill label="Pending" bg="#F9FAFB" text="#94A3B8"/>
                            }
